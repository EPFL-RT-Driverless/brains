...
from brains_python.vision.yolo.utils.torch_utils import torch_distributed_zero_first
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def cache_labels(
        self, path=Path("./labels.cache"), prefix="", kpt_label=False, nkpt=7
    ):
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        ...
                        if kpt_label:
                            assert (
                                l.shape[1] == nkpt * 3 + 5
                            ), "labels require {} columns each".format(nkpt * 3 + 5)
                            assert (
                                l[:, 5::3] <= 1
                            ...
                            kpts = np.zeros((l.shape[0], nkpt * 2 + 5))
                        ...
                        ne += 1  # label empty
                        l = (
                            np.zeros((0, nkpt * 2 + 5), dtype=np.float32)
                            if kpt_label
    ...
    if n:
        ...
        if use_segments:  # warp segments
            ...
            if kpt_label:
                ...
                x_kpts = xy_kpts[:, list(range(0, nkpt * 2, 2))]
                y_kpts = xy_kpts[:, list(range(1, nkpt * 2, 2))]
                ...
                xy_kpts[:, list(range(0, nkpt * 2, 2))] = x_kpts
                xy_kpts[:, list(range(1, nkpt * 2, 2))] = y_kpts
