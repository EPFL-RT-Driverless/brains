...
from brains_python.vision.yolo.utils.general import bbox_iou
from brains_python.vision.yolo.utils.torch_utils import is_parallel
...
class ComputeLoss:
    # Compute losses
    def __init__(self, model, autobalance=False, kpt_label=False, nc=4, nkpt=7):
        super(ComputeLoss, self).__init__()
        # print('Compute loss: kpt_label: {}, npkt: {}'.format(kpt_label, nkpt))
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        sigmas = (
            torch.tensor([0.25, 0.35, 0.79, 0.72, 0.62, 1.07, 0.87], device=device)
            / 10.0
        )
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.kpt_label:
                    ...
                    pkpt_x = ps[:, (self.nc + 5) :: 3] * 2.0 - 0.5
                    # pkpt_x = ps[:, 6::3] * 2. - 0.5
                    pkpt_y = ps[:, (self.nc + 5 + 1) :: 3] * 2.0 - 0.5
                    # pkpt_y = ps[:, 7::3] * 2. - 0.5
                    pkpt_score = ps[:, (self.nc + 5 + 2) :: 3]
                    # pkpt_score = ps[:, 8::3]
                    ...
                    lkpt += (
                        kpt_loss_factor * (torch.log(d + 1 + 1e-9) * kpt_mask).mean()
                    )  # More stable loss function to try prevent kpts out of bounding box see: https://github.com/WongKinYiu/yolov7/pull/501
                # Objectness
    ...
    def build_targets(self, p, targets):
        ...
        if self.kpt_label:
            ...
            gain = torch.ones(
                5 + 2 * (self.nkpt + 1), device=targets.device
        ...
        for i in range(self.nl):
            ...
            if self.kpt_label:
                ...
                gain[2 : int((5 + 2 * (self.nkpt + 0.5)))] = torch.tensor(p[i].shape)[
                    (self.nkpt + 2) * [3, 2]
                ]  # xyxy gain
            else:
        ...
        # print('Tkpt: {}'.format(len(tkpt)))
        # print([len(a) for a in tkpt])
