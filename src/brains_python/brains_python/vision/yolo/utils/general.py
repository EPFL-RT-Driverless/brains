...
from brains_python.vision.yolo.utils.metrics import fitness
from brains_python.vision.yolo.utils.torch_utils import init_torch_seeds

__all__ = ["check_img_size", "non_max_suppression", "scale_coords"]
...
def non_max_suppression(
    ...
    Returns:
         list of detections, on (n,6) tensor per image [xyxy, conf, cls] if not kpt_label
         Else list of detections, on (n,6 + 3*nkpt) tensor per image [xyxy, conf, cls, kpt_x, kpt_y, kpt_score, ...]
    """
    ...
    max_det = 5000  # maximum number of detections per image
    ...
    output = [
        torch.zeros((0, 5 + nc + 3 * nkpt), device=prediction.device)
    ] * prediction.shape[0]
    # print(len(output))
...
def non_max_suppression_export(
    ...
    min_wh, max_wh = 2, 4096  # (pixels) minimum and maximum box width and height
    # xc = prediction[..., 4] > conf_thres  # candidates
    # comp_tensor = torch.full(len(prediction[..., 4]), conf_thres)
    xc = torch.where(prediction[..., 4] > conf_thres, 1, 0)
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        x = torch.cat((box, conf, j.float(), kpts), 1)[
            torch.where(conf.view(-1) > conf_thres, 1, 0)
        ]
        # x = torch.cat((box, conf, j.float(), kpts), 1)[conf.view(-1)]
        c = x[:, 5:6] * (0 if agnostic else max_wh)  # classes
        boxes, scores = x[:, :4] + c, x[:, 4]  # boxes (offset by class), scores
        # boxes_ = einops.repeat(boxes, 'num bb -> num cls bb', cls=nc).squeeze()
        # scores = einops.repeat(scores, 'sc num -> sc num cls', cls=nc)
        i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS
        output[xi] = x[i]
    return output
