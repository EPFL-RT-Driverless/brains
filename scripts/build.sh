...
PYTHONWARNINGS=ignore:::setuptools.command.install,ignore:::setuptools.command.easy_install,ignore:::pkg_resources \  # colcon uses deprecated setuptools functions so we need to suppress warnings for it to successfully build the workspace
  colcon build --symlink-install --cmake-args "-DPython3_EXECUTABLE=$PYTHON_EXE"
